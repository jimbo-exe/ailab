...
open_stack=[] #this acts like a queue.
closed_list=[]
...
def dfs(p1): #this performs best first search.
    open_stack.append((p1.heuristic_misplacedTiles(),p1))

    while open_stack!=[]:
        open_stack.sort(key=lambda x: x[0]) #sorts on the basis of first value in tuple, that is, heuristic value.
        _,current=open_stack.pop(0)#for bfs open_stack.pop(0). for dfs open_stack.pop()
        ...
        if (new_state.down() and CheckClosedList(new_state)==False):
            open_stack.append((new_state.heuristic_misplacedTiles(),new_state))
